...
const EditCase = React.forwardRef((props, ref) => {
	const { open, onClose, editedCase, fetchCaseData } = props;

	const model = Schema.Model({
		preview: Schema.Types.ArrayType().of(
		...
		blocks: Schema.Types.ArrayType().of(
			Schema.Types.ObjectType().shape({
				...
				img: Schema.Types.ArrayType()
					.addRule((value, data) => {
						const { img, oldImg } = data;
						if (!oldImg && img.length < 1) {
							return false;
						}
				...
				gallery: Schema.Types.ArrayType()
					.addRule((value, data) => {
						...
						if ((gallery?.length || 0) + (oldGallery?.length || 0) < 2) {
							return false;
						}
						return true;
					}, 'В галереи должно быть минимум 2 изображения!')
					.of(
						Schema.Types.ObjectType().shape({
							blobFile: Schema.Types.ObjectType().shape({
								name: Schema.Types.StringType().pattern(
	...
	useEffect(() => {
		if (editedCase && editedCase.title) {
			setIsReady(true);
			setIsEditable(false);
			let blocks = [];
			const oldBlocks = editedCase.blocks;
			if (oldBlocks && oldBlocks.length > 0) {
				oldBlocks.map((block, bindex) => {
					let field = {
						type: block.type,
					};
					if (block.type === 'img') {
			...
			setFormValue({
				preview: [],
				mobileWidth: editedCase.mobileWidth || 2,
				titleRu: editedCase.title?.ru || '',
				titleEn: editedCase.title?.en || '',
				titleUz: editedCase.title?.uz || '',
		...
		if (formRef.current.check()) {
			...
			if (formValue.blocks && formValue.blocks.length > 0) {
				...
				blocks.map((b, index) => {
					...
					if (b.type === 'video') {
						...
						let updatedoldKeys = [];
						let keys = [];
						if (updatedOldGallery && updatedOldGallery.length > 0) {
							updatedoldKeys = updatedOldGallery;
						}
						if (gallery && gallery.length > 0) {
							gallery.map((g, gindex) => {
								const key = `gallery_${index}_${gindex + updatedoldKeys.length}`;
								const img = {
									key: key,
									img: g.blobFile || null,
								};
						...
						blocksKey.keys = keys;
						blocksKey.updatedOldKeys = updatedoldKeys;
					} else {
			...
			try {
				const { data } = await axios.put(
					`/api/cases/cases/${editedCase._id}`,
					formData,
					config
				);
	...
	return (
		<Drawer ref={ref} open={open} onClose={onClose} size="lg" full>
			<Drawer.Header>
				<Drawer.Title>Редактирование кейса</Drawer.Title>
			</Drawer.Header>
			<Drawer.Body>
				<FormWrapper
				...
				>
					...
					<FileUploader
						label="Превью кейса"
						name="preview"
						oldImg={editedCase?.preview || null}
...
const BlockItem = ({
	...
	const handleGalleryDelete = (index) => {
		const oldGallery = rowValue.oldGallery.filter((g, i) => i !== index);
		onChange(rowIndex, { ...rowValue, oldGallery: oldGallery });
	};
			...
			<Cell>
				...
				{type === 'img' && (
					<FileUploaderAlt
						...
						disabled={disabled || rowValue.img?.length > 0}
						oldImg={rowValue.oldImg || null}
						onChange={(list) => {
				...
				{type === 'gallery' && (
					<FileUploaderAlt
						label="Галлерея"
						oldGallery={rowValue.oldGallery}
						onGalleryDeleteClick={(index) => handleGalleryDelete(index)}
						onChange={(list) => {
							onChange(rowIndex, { ...rowValue, gallery: list });
						}}
						popoverProps={{
							text:
								'Блок с 2 или больше изображениями. Чтобы удалить изображение из текущей галереи нажмите на значок на изображении. Чтобы загрузить новое изображение в галерею нажмите сюда',
						}}
						errExt={galleryErrors.ext || null}
