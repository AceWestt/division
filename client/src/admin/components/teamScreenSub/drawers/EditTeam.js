...
const EditTeam = React.forwardRef((props, ref) => {
	const { open, onClose, team, fetchData } = props;
	...
	useEffect(() => {
		if (team && team.name?.ru) {
			...
			setFormValue({
				...
				nameEn: team.name.en,
				nameUz: team.name.uz,
				titleRu: team.title.ru,
				titleEn: team.title.en,
				titleUz: team.title.uz,
			});
		}
	}, [team]);
	...
	const onSubmit = async () => {
		if (formRef.current.check()) {
			...
			try {
				const { data } = await axios.put(`/api/team/${team._id}`, formData, config);
				...
				if (error.response.data.error) {
					...
					toaster.push(errorMessage('Не удалось обновить сотрудника!'), {
						placement: messagePlacement,
					});
				}
			}
	...
	return (
		<Drawer ref={ref} open={open} onClose={onClose} size="lg" full>
			<Drawer.Header>
				<Drawer.Title>Редактирование сотрудника</Drawer.Title>
			</Drawer.Header>
			<Drawer.Body>
				<FormWrapper
